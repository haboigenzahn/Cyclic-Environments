...
"""
This code is a variation of the ParameterFitting-main.py file included in https://github.com/haboigenzahn/OoL-KineticParameterEstimation
which has been modified to fit the glycine homopolymer system. See that repository for details and requirements. 
"""
...
# FILE UPDATE
filename = "Mechanism1_TrainingData"
df = pd.read_csv("Data/{}.csv".format(filename))
print(df.shape)

# Define initial parameter guess
params = 1.0*np.ones(8)
newname =  filename # new name for running the same data file with different ICs/bounds
...
df.to_csv("Params/{}_Params.csv".format(newname), index=False)
...
df_COV.to_csv("Params/{}_COV.csv".format(newname), index=False)
...
# param_filename = 'Gly24_Mechanism1_plusIter1d'
